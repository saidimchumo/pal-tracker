...
credentials=`cf curl /v2/apps/$app_guid/env | jq '.system_env_json.VCAP_SERVICES | if .["p-mysql"] != null then .["p-mysql"] elif .["p.mysql"] != null else .["cleardb"] end | .[0].credentials'`
